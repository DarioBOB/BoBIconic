...
export class LoggerService {
  ...
  private logToConsole(entry: LogEntry): void {
    ...
    const memoryStr = entry.memoryUsage ? ` | Memory: ${entry.memoryUsage}MB` : '';
    const requestStr = entry.requestId ? ` | Request: ${entry.requestId}` : '';

    const message = `[${timestamp}] ${levelStr} | ${categoryStr} | ${entry.message}${contextStr}${memoryStr}${requestStr}`;
  ...
  private logToRemote(entry: LogEntry): void {
    // Implémentation future pour envoi à un service de monitoring
