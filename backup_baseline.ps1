...
Write-Host "ATTENTION : Ne JAMAIS commiter de secrets (tokens) dans le dépôt !" -ForegroundColor Yellow

# Juste avant le git add ., supprime les secrets du dossier s'ils existent
if (Test-Path "github_token.txt") { Remove-Item "github_token.txt" }
if (Test-Path "gitlab_token.txt") { Remove-Item "gitlab_token.txt" }
...
# Sécurité supplémentaire : retire toute trace des tokens de l'historique
& git filter-branch --force --index-filter "git rm --cached --ignore-unmatch github_token.txt gitlab_token.txt" --prune-empty --tag-name-filter cat -- --all
