...
// Les imports TranslatePipe, TranslateService, DemoService doivent être adaptés selon leur disponibilité
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, IonicModule, FormsModule, TranslateModule, TranslatePipe],
...
export class EmailAuthPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.password = '';
    this.errorMessage = '';
    // Initialisation de la langue (à adapter si TranslateService est disponible)
  ...
  async login() {
    ...
    // Désactiver le mode démo à chaque login classique (à adapter si DemoService est disponible)
  ...
  async loadDemo() {
    // À adapter si DemoService est disponible
    // try {
