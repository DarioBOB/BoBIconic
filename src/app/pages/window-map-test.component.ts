...
export class WindowMapTestComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  polylineDone: any = null;
  polylineTodo: any = null;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if ((changes['segments'] || changes['currentPercent']) && this.map) {
      this.drawTrajectory();
    }
  }
  ...
  drawTrajectory() {
    if (!this.map) return;
    if (this.polylineDone) this.map.removeLayer(this.polylineDone);
    if (this.polylineTodo) this.map.removeLayer(this.polylineTodo);
    if (!this.segments || this.segments.length < 2) {
      console.log('[window-map-test] Pas de segments à dessiner', this.segments);
      return;
    }
    // Découper les segments selon la progression
    const idx = Math.round(this.currentPercent);
    const donePoints = this.segments.slice(0, idx + 1).map(s => [s.lat, s.lng]);
    const todoPoints = this.segments.slice(idx).map(s => [s.lat, s.lng]);
    console.log('[window-map-test] Trace done:', donePoints.length, 'Trace todo:', todoPoints.length);
    this.polylineDone = L.polyline(donePoints, { color: 'blue', weight: 4 }).addTo(this.map);
    this.polylineTodo = L.polyline(todoPoints, { color: 'blue', weight: 4, dashArray: '8,8' }).addTo(this.map);
  }
  ...
  updatePOIMarkers() {
    ...
    for (const poi of this.poisVisibles) {
      const lat = (typeof poi.lat === 'number' && !isNaN(poi.lat)) ? poi.lat : (typeof poi.latitude === 'number' && !isNaN(poi.latitude) ? poi.latitude : undefined);
      const lon = (typeof poi.lon === 'number' && !isNaN(poi.lon)) ? poi.lon : (typeof poi.longitude === 'number' && !isNaN(poi.longitude) ? poi.longitude : undefined);
      ...
      marker.bindPopup(`<b>${poi.name}</b><br>${poi.description}<br><a href='${poi.wiki_url}' target='_blank'>Wikipedia</a>`);
  ...
  ngOnDestroy() {
    ...
    if (this.polylineDone) this.polylineDone.remove();
    if (this.polylineTodo) this.polylineTodo.remove();
  }
} 