...
import { GeoStatusService, GeoMode } from '../../services/geo-status.service';
import { TrajectoryService, TrajectoryPoint } from '../../services/trajectory.service';
...
export class WindowMapPage implements OnInit, OnDestroy {
  ...
  public currentMode: GeoMode = 'DEMO';
  ...
  async ngOnInit() {
    ...
    console.log('[DEBUG] Mode détecté dans ngOnInit:', this.currentMode);
    this.windowService.setMode(this.currentMode);
    await this.loadSegmentsForMode(this.currentMode);
  ...
  private updateAircraftPosition(position: { lat: number; lng: number }) {
  ...
  private async loadSegmentsForMode(mode: GeoMode) {
    if (mode === 'REAL') {
      this.startGpsWatch();
    } else {
      this.stopGpsWatch();
  ...
  private async startGpsWatch() {
    ...
    this.gpsWatchId = Geolocation.watchPosition({}, (position: Position | null, err?: any) => {
      ...
      segments.push(pt);
      this.windowService.setRealSegments([...segments]);
      this.resetGpsTimeout();
    });
    this.resetGpsTimeout();
  }

  private stopGpsWatch() {
    if (this.gpsWatchId) {
  ...
  private resetGpsTimeout() {
    if (this.gpsTimeout) clearTimeout(this.gpsTimeout);
    this.gpsTimeout = setTimeout(() => {
      // Timeout atteint : basculer en mode déconnecté
      this.currentMode = 'DISCONNECTED';
      this.windowService.setMode('DISCONNECTED');
      this.loadSegmentsForMode('DISCONNECTED');
    }, WindowMapPage.GPS_TIMEOUT_MS);
  }
} 