...
export class FlightEnrichmentService {
  ...
  /**
   * Enrichit les données du vol GVA->ATH le plus proche dans le temps (démo dynamique)
   */
  enrichLatestGvaToAthFlight(mode: 'time' | 'distance' = 'time'): Observable<TrackPoint[]> {
    return this.openSkyService.findLatestGvaToAthFlight().pipe(
      switchMap(flight => {
        console.log('[OpenSky] Vérification vol transmis:', flight, typeof flight?.icao24, typeof flight?.firstSeen);
        if (!flight || typeof flight.icao24 !== 'string' || !flight.icao24.trim() || typeof flight.firstSeen !== 'number' || isNaN(flight.firstSeen)) {
          throw new Error('Aucun vol GVA->ATH trouvé ou données incomplètes');
        }

        // Calculer la distance approximative Genève-Athènes
        const distance = 1800; // ~1800km
        const duration = flight.lastSeen - flight.firstSeen;

        // Utiliser la nouvelle méthode pour obtenir la trajectoire complète
        return this.openSkyService.getCompleteFlightTrack(
          flight.icao24,
          flight.firstSeen,
          flight.lastSeen,
          distance
      ...
      tap(points => {
        console.log('[OpenSky] TrackPoints générés par enrichLatestGvaToAthFlight:', points);
      }),
      catchError(err => {
