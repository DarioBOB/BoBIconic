...
export class DemoService {
  private readonly DEMO_EMAIL = 'guestuser@demo.com';
  private readonly DEMO_PASSWORD = 'DemoPassword123!';
  private readonly DEMO_USER_ID = 'guest-demo';
  ...
  async activateDemo(lang: string): Promise<boolean> {
    try {
      ...
      const userRef = doc(this.firestore, `users/${this.DEMO_USER_ID}`);
  ...
  async resetDemoTrips() {
    ...
    try {
      ...
      // Infos aéroports
      const gva = { code: 'GVA', city: 'Genève', name: 'Aéroport de Genève', tz: 'Europe/Zurich', lat: 46.2381, lon: 6.1089 };
      const ath = { code: 'ATH', city: 'Athènes', name: 'Aéroport d\'Athènes Elefthérios-Venizélos', tz: 'Europe/Athens', lat: 37.9364, lon: 23.9475 };
      // Trip multilingue
      const trip = {
